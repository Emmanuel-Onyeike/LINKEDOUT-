...
if (likesError) {
    ...
    // → we continue anyway, just treat as no likes
}
// ── Fetch which of these posts the current user has reposted ───────────
const { data: userReposts, error: repostsError } = await supabase
    ...
    .eq('user_id', currentUserId);
if (repostsError) {
    console.warn("Could not fetch user reposts:", repostsError);
    // → continue, treat as no reposts
}
    ...
    return `
        <div class="bg-white border border-slate-200 rounded-[32px] overflow-hidden shadow-sm">
            ...
            <div class="flex items-center gap-6 px-6 pb-5 pt-3 border-t border-slate-100">
                <button
                    onclick="handleLike('${post.id}', this, ${isLiked})"
                ...
                <button
                    onclick="handleRepost('${post.id}', ${isReposted})"
...
async function handleLike(postId, btn, alreadyLiked) {
    ...
    // Background sync (optional – can be throttled if too frequent)
    // renderFeed();
}
async function handleRepost(postId, alreadyReposted) {
    ...
    if (!error) {
        ...
        renderFeed(); // or just update count locally if you prefer
    } else {
